...
// The room's own baked textures (inside the .glb) are usually the biggest memory hog.
// If the page still goes black on a phone, lower the mobile value to 1024.
const MAX_MODEL_TEXTURE_SIZE = isMobile ? 2048 : 4096;

function downscaleTextureIfNeeded(texture, maxSize = MAX_TEXTURE_SIZE) {
...
function setupModel(gltf) {
    ...
    // Textures are uploaded to the GPU lazily on first render, so shrinking them
    // here (before scene.add) keeps huge baked textures from exhausting iPhone memory.
    const seenTextures = new Set();
    const TEXTURE_SLOTS = ['map', 'lightMap', 'aoMap', 'emissiveMap', 'normalMap', 'roughnessMap', 'metalnessMap', 'alphaMap'];
    model.traverse((child) => {
        ...
        mats.forEach((mat) => {
            ...
            TEXTURE_SLOTS.forEach((slot) => {
                const tex = mat[slot];
                if (tex && !seenTextures.has(tex.uuid)) {
                    seenTextures.add(tex.uuid);
                    const before = tex.image && Math.max(tex.image.width || 0, tex.image.height || 0);
                    downscaleTextureIfNeeded(tex, MAX_MODEL_TEXTURE_SIZE);
                    debugLog('Model texture ' + slot + ': ' + before + 'px');
                }
            });
        });
    });
